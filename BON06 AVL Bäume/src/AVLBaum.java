public class AVLBaum<T extends Comparable<T>>
{
	...
	public boolean suchen(T daten)
	{
		...
		AVLKnoten<T> aktKnoten = getWurzel();
		int cmp = daten.compareTo(aktKnoten.getDaten());

		while (aktKnoten != null)
		{
			cmp = daten.compareTo(aktKnoten.getDaten());

			if (cmp == 0)
				return true;
			else if (cmp < 0)
				aktKnoten = aktKnoten.getKnotenLinks();
			else
				aktKnoten = aktKnoten.getKnotenRechts();
		}
		return false;
	}
	...
	private AVLKnoten<T> linksAusgleich(AVLKnoten<T> w)
	{
		...
		switch (w.getBalance())
		{
			...
			case -1: // Ausgleich notwendig
			{
				...
				else if (p.getBalance() == +1) // Fälle 2a und 2b
				{
